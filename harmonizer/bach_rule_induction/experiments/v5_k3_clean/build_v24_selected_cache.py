#!/usr/bin/env python3
"""Build exact worlds for the retained V23 model plus the V24 residual group."""
...
def selected_features(
    *,
    ...
    if len(group_features) != int(group["size"]):
        raise ValueError("V24 residual group has the wrong frozen size")
    features = tuple(
        [by_key[feature.key] for feature in baseline_features]
        + group_features
    )
    if len({feature.key for feature in features}) != len(features):
        raise ValueError("V24 selected features contain duplicates")
    return features
...
def main() -> int:
    ...
    print(
        f"[v24-cache] features={len(features)} "
        f"train={train['chosen'].size} validation={validation['chosen'].size}",
        flush=True,
    )
    print(f"[v24-cache] wrote {args.cache}", flush=True)
    return 0
