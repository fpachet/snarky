...
def _sample_one(
    ...
    try:
        return k3.rhythmic_gibbs_sample(
            chain.blocks,
        ...
        raise ValueError(f"{chain.piece_id}: {error}") from error
...
def _sample(
    ...
    *,
    ...
    generated = (
        map(_sample_one, tasks)
        if executor is None
        else executor.map(_sample_one, tasks)
    )
    for chain, blocks in zip(chains, generated, strict=True):
        chain.blocks = blocks
...
def _markdown(result: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        (
            f"- Pièces structurellement exclues avant sélection : "
            f"`{len(calibration['eligibility_exclusions'])}`."
        ),
        f"- Époques : `{calibration['epochs']}`.",
        f"- Processus d'échantillonnage : `{calibration['workers']}`.",
        (
...
def main() -> int:
    ...
    chains: list[generative.Chain] = []
    train_ids: list[str] = []
    eligibility_exclusions: list[dict[str, str]] = []
    for piece_id in candidate_train_ids:
        lattice = k3.extract_piece_lattice(
            generative._score_path(args.scores, piece_id),
            piece_id,
        )
        try:
            k3.validated_attack_segments(lattice.blocks, lattice.attacks)
        except ValueError as error:
        ...
        if len(train_ids) == args.train_pieces:
            break
    if len(train_ids) != args.train_pieces:
        raise ValueError(
            f"Only {len(train_ids)} train pieces satisfy the rhythmic boundary "
            f"contract; requested {args.train_pieces}"
        )
