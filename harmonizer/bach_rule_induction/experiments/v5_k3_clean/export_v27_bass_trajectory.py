#!/usr/bin/env python3
"""Export the confirmed V27 bass-trajectory model and Snarky factors."""
...
DEFAULT_BASELINE = FACTOR_BASE / "v26_conditional_candidate_model.json"
DEFAULT_FIT = FACTOR_BASE / "v27_bass_trajectory_confirmation50.json"
DEFAULT_MODEL = FACTOR_BASE / "v27_bass_trajectory_full_model.json"
DEFAULT_CATALOGUE = FACTOR_BASE / "v27_bass_trajectory_full_factors.yaml"
DEFAULT_PROGRAM = FACTOR_BASE / "v27_bass_trajectory_full.factors"
...
def _factor_record(rule: dict[str, Any]) -> dict[str, Any]:
    ...
    is_v27 = rule["feature"]["kind"] == "central_bass_trajectory_status"
    record = {
        "id": rule["id"],
        "family": rule["family"],
        ...
        "member_role": (
            "bass_trajectory_status" if is_v27 else "retained_v26_baseline"
        ),
    }
    if is_v27:
        record["rule_group"] = "RG-LEARNED-V27-001"
    return record
...
def main() -> int:
    ...
    if not fit["selection"]["group_retained"]:
        raise ValueError("V27 was not confirmed by the frozen protocol")
    ...
    if len(weights) != baseline_count + len(features):
        raise ValueError("V27 selected parameter vector has the wrong size")

    candidate = json.loads(json.dumps(baseline))
    candidate["experiment"] = {
        **baseline["experiment"],
        "id": "K3-V27-BASS-TRAJECTORY-FULL-1",
        ...
        "new_bass_trajectory_factor_count": len(features),
    ...
    for rule, weight in zip(
        ...
        rule["weight"] = weight
        rule["origin"] = "joint_conditional_mle_v27_refit"
    candidate["model"]["rules"].extend(
        {
            "id": f"F-K3-V27-{index:03d}",
            "family": "bass_trajectory_status",
    ...
    catalogue = {
        "schema_version": 1,
        "id": "K3-V27-CONFIRMED-BASS-TRAJECTORY",
        "model_id": candidate["experiment"]["id"],
        "source_model": str(args.model.resolve()),
        "factor_group": "k3_v27_confirmed_bass_trajectory",
        "status": "CONDITIONALLY_CONFIRMED",
        "counts": {
            "canonical_factors_after_merge": len(factors),
            "retained_v26_factors": baseline_count,
            "new_bass_trajectory_cells": len(features),
    ...
    program = render_factor_program(
        catalogue,
        group_name="k3_v27_confirmed_bass_trajectory",
        source_label=args.catalogue.name,
    )
    (parsed,) = parse_factor_groups(program)
    if len(parsed.factors) != len(factors):
        raise ValueError("V27 Snarky program lost factors during parsing")
    ...
    print(f"[v27-export] wrote {args.model}", flush=True)
    print(f"[v27-export] wrote {args.catalogue}", flush=True)
    print(f"[v27-export] wrote {args.program}", flush=True)
    return 0
