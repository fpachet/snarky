#!/usr/bin/env python3
"""Audit the frozen V26 weak-role × resolution vocabulary before fitting."""
...
DEFAULT_CONFIG = FACTOR_BASE / "v26_joint_weak_resolution_config.yaml"
...
DEFAULT_OUTPUT = FACTOR_BASE / "v26_joint_weak_resolution_coverage.json"
DEFAULT_REPORT = FACTOR_BASE / "V26_JOINT_WEAK_RESOLUTION_COVERAGE.md"
...
def _markdown(result: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- Blocs faibles résiduels : `{experiment['residual_weak_blocks']}`.",
        f"- Partition exhaustive : "
        f"`{str(experiment['all_residual_blocks_have_one_status']).lower()}`.",
        "",
...
def main() -> int:
    ...
    chosen = k3.central_joint_weak_resolution_statuses(
        context,
        context.chosen_pitches[:, None],
    )[:, 0]
    alternatives = k3.central_joint_weak_resolution_statuses(context)
    weak_rows = context.metric_levels < 2
    coverage = config["coverage"]
    cells = []
    for status, label in enumerate(k3.JOINT_WEAK_RESOLUTION_NAMES):
        authentic = representatives & (chosen == status)
        opportunities = weak_rows[:, None] & (alternatives == status)
    ...
    residual = representatives & (chosen >= 0)
    result = {
        "experiment": {
            "id": config["id"],
            ...
            "all_residual_blocks_have_one_status": bool(
                residual.sum() == sum(cell["authentic_unique_blocks"] for cell in cells)
            ),
    ...
    print(
        f"[v26-coverage] eligible={result['eligible_cell_count']}/"
        f"{result['cell_count']} residual={int(residual.sum())}",
        flush=True,
    )
    print(f"[v26-coverage] wrote {args.output}", flush=True)
    print(f"[v26-coverage] wrote {args.report}", flush=True)
    return 0
