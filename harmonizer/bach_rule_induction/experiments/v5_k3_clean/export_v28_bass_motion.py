#!/usr/bin/env python3
"""Export the confirmed V28 bass-motion model and Snarky factors."""
...
DEFAULT_BASELINE = FACTOR_BASE / "v27_bass_trajectory_full_model.json"
DEFAULT_FIT = FACTOR_BASE / "v28_bass_motion_confirmation50.json"
DEFAULT_MODEL = FACTOR_BASE / "v28_bass_motion_full_model.json"
DEFAULT_CATALOGUE = FACTOR_BASE / "v28_bass_motion_full_factors.yaml"
DEFAULT_PROGRAM = FACTOR_BASE / "v28_bass_motion_full.factors"
...
def _factor_record(rule: dict[str, Any]) -> dict[str, Any]:
    ...
    is_v28 = rule["feature"]["kind"] == "central_bass_motion_status"
    record = {
        "id": rule["id"],
        "family": rule["family"],
        ...
        "member_role": "bass_motion_status" if is_v28 else "retained_v27_baseline",
    }
    if is_v28:
        record["rule_group"] = "RG-LEARNED-V28-001"
    return record
...
def main() -> int:
    ...
    if not fit["selection"]["group_retained"]:
        raise ValueError("V28 was not confirmed by the frozen protocol")
    ...
    if len(weights) != baseline_count + len(features):
        raise ValueError("V28 selected parameter vector has the wrong size")

    candidate = json.loads(json.dumps(baseline))
    candidate["experiment"] = {
        **baseline["experiment"],
        "id": "K3-V28-BASS-MOTION-FULL-1",
        ...
        "new_bass_motion_factor_count": len(features),
    ...
    for rule, weight in zip(
        ...
        rule["weight"] = weight
        rule["origin"] = "joint_conditional_mle_v28_refit"
    candidate["model"]["rules"].extend(
        {
            "id": f"F-K3-V28-{index:03d}",
            "family": "bass_motion_status",
    ...
    catalogue = {
        "schema_version": 1,
        "id": "K3-V28-CONFIRMED-BASS-MOTION",
        "model_id": candidate["experiment"]["id"],
        "source_model": str(args.model.resolve()),
        "factor_group": "k3_v28_confirmed_bass_motion",
        "status": "CONDITIONALLY_CONFIRMED",
        "counts": {
            "canonical_factors_after_merge": len(factors),
            "retained_v27_factors": baseline_count,
            "new_bass_motion_cells": len(features),
    ...
    program = render_factor_program(
        catalogue,
        group_name="k3_v28_confirmed_bass_motion",
        source_label=args.catalogue.name,
    )
    (parsed,) = parse_factor_groups(program)
    if len(parsed.factors) != len(factors):
        raise ValueError("V28 Snarky program lost factors during parsing")
    ...
    print(f"[v28-export] wrote {args.model}", flush=True)
    print(f"[v28-export] wrote {args.catalogue}", flush=True)
    print(f"[v28-export] wrote {args.program}", flush=True)
    return 0
