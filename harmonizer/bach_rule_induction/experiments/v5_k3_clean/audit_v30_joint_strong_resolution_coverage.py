#!/usr/bin/env python3
"""Audit V30 strong-role × following-sonority coverage before fitting."""
...
DEFAULT_CONFIG = FACTOR_BASE / "v30_joint_strong_resolution_config.yaml"
...
DEFAULT_OUTPUT = FACTOR_BASE / "v30_joint_strong_resolution_coverage.json"
DEFAULT_REPORT = FACTOR_BASE / "V30_JOINT_STRONG_RESOLUTION_COVERAGE.md"
...
def _markdown(result: dict[str, Any]) -> str:
    lines = [
        "# V30 — couverture forte résiduelle × résolution",
        "",
...
def main() -> int:
    ...
    chosen = k3.central_joint_strong_resolution_statuses(
        context,
        context.chosen_pitches[:, None],
    )[:, 0]
    alternatives = k3.central_joint_strong_resolution_statuses(context)
    coverage = config["coverage"]
    cells = []
    for status, label in enumerate(k3.JOINT_STRONG_RESOLUTION_NAMES):
    ...
    result = {
        "experiment": {
            ...
            "residual_strong_blocks": int(residual.sum()),
    ...
    print(
        f"[v30-coverage] eligible={result['eligible_cell_count']}/"
